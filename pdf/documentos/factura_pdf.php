...
//echo "select * from tmp where session_id='".$session_id."'";
 ...
 while($rw= mysqli_fetch_assoc($sql_count)){
     ...
     //$numero_factura = $_SESSION['id_factura'];

//     $sql_insert_details = "INSERT INTO detalle_factura (numero_factura, id_producto,cantidad,precio_venta)
//                            VALUES ('$numero_factura','$id','$cantidad','$precio_venta')";
//     mysqli_query($con, $sql_insert_details);
 }

	$count=mysqli_num_rows($sql_count);
	if ($count==0)
	{
	echo "<script>alert('Não existem produtos adicionados à factura')</script>";
